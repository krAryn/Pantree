...
const getRandom = (image) => {
    // return image[Math.floor(Math.random() * image.length)];
    return image[0]
}

const ProductCard = ({prod}) => {   
    const { addToCart, removeFromCart, cartItems, navigate, searchQuery } = useAppContext()
            ...
            <div className="text-gray-500/60 text-sm">
                ...
                <div className="flex items-center gap-0.5">
                    {Array(5).fill('').map((_, i) => (
                        <img key={i} className='md:w-3.5 w-3' src={i < product.rating? assets.star_icon: assets.star_dull_icon} alt="" />
                    ))}
                    <p>({product.rating})</p>
                </div>
                <div className="flex items-end justify-between mt-3">
                    ...
                    <div className="text-primary" onClick={e => e.stopPropagation()}>
                        {!cartItems[product._id] ? (
                            <button className="cursor-pointer flex items-center justify-center gap-1 bg-primary/10 border border-primary/40 md:w-[80px] w-[64px] h-[34px] rounded" onClick={() => {addToCart(product._id)}} >
