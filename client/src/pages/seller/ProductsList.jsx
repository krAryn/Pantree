...
const ProductsList = () => {
    ...
    const fetchOutOfStockProducts = () => {
        return dummyProducts.filter(product => !product.inStock)
    }

    const [outOfStockProducst, setOutOfStockProducst] = useState(fetchOutOfStockProducts())

    useEffect(() => {
        setOutOfStockProducst(fetchOutOfStockProducts())
    }, [products])

    return products && (
        <div className="no-scrollbar flex-1 flex flex-col justify-between h-[90vh] overflow-y-auto">
                ...
                <div className="flex flex-col items-center max-w-4xl w-full overflow-hidden rounded-md bg-white border border-gray-500/20">
                    <table className="md:table-auto table-fixed w-full">
                        ...
                        <tbody className="text-sm text-gray-500">
                            {[...products, ...outOfStockProducst].map((product) => (
